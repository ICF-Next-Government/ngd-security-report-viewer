...
<body class="antialiased">
  ...
  <div id="main-content" class="min-h-screen">
    ...
    <section aria-labelledby="summary-heading" class="container mx-auto px-4 py-8">
    ...
    <!-- Findings section -->
    <main role="main" aria-labelledby="findings-heading" class="container mx-auto px-4 py-8">
      <h2 id="findings-heading" class="sr-only">Security Findings</h2>
      ${findingsSection}
    </main>

    <!-- Footer -->
    <footer role="contentinfo" class="container mx-auto px-4 py-8 mt-16 border-t border-slate-700">
      <div class="text-center text-sm text-slate-400 space-y-2">
        ...
        ${formattedTimestamp ? `<p>Report generated: ${escapeHtml(formattedTimestamp)}</p>` : ""}
        <p class="text-xs">
          This report contains ${results.length} security finding${results.length !== 1 ? "s" : ""}
          ${hasGroups ? `grouped into ${deduplicationGroups.length} unique issue${deduplicationGroups.length !== 1 ? "s" : ""}` : ""}
          across ${summary.filesAffected} file${summary.filesAffected !== 1 ? "s" : ""}.
        </p>
      </div>
    </footer>
  </div>
