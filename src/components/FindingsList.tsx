...
import { Search, Filter, ChevronDown, FileText, AlertTriangle, Info, CheckCircle } from 'lucide-react';
...
export const FindingsList: React.FC<FindingsListProps> = ({ results }) => {
  ...
  const severityColors = {
    critical: { bg: 'bg-red-900/20', text: 'text-red-300', border: 'border-red-700', icon: 'text-red-400', badge: 'bg-red-900/40 text-red-300 border-red-700' },
    high: { bg: 'bg-orange-900/20', text: 'text-orange-300', border: 'border-orange-700', icon: 'text-orange-400', badge: 'bg-orange-900/40 text-orange-300 border-orange-700' },
    medium: { bg: 'bg-amber-900/20', text: 'text-amber-300', border: 'border-amber-700', icon: 'text-amber-400', badge: 'bg-amber-900/40 text-amber-300 border-amber-700' },
    low: { bg: 'bg-blue-900/20', text: 'text-blue-300', border: 'border-blue-700', icon: 'text-blue-400', badge: 'bg-blue-900/40 text-blue-300 border-blue-700' },
    info: { bg: 'bg-slate-800/50', text: 'text-slate-300', border: 'border-slate-600', icon: 'text-slate-400', badge: 'bg-slate-700/50 text-slate-300 border-slate-600' },
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <span className="text-slate-400">{filteredResults.length} findings</span>
      </div>
        ...
        {filteredResults.map((result) => {
          ...
          return (
            <div
              ...
              onClick={() => setSelectedFinding(selectedFinding?.id === result.id ? null : result)}
            >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-3">
                      <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ${colors.badge} border`}>
                        {result.severity.toUpperCase()}
                      </span>
                      <span className="text-sm text-slate-400">{result.ruleId}</span>
                    </div>
                    <ChevronDown className={`h-5 w-5 text-slate-400 transition-transform ${selectedFinding?.id === result.id ? 'rotate-180' : ''}`} />
                  </div>
